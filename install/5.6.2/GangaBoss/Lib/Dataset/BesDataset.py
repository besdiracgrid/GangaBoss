...
class BesDataset(Dataset):
    '''Class for handling data sets (i.e. inputdata for jobs).

    Example Usage:
    ds = BesDataset(["lfn:/some/lfn.file","pfn:/some/pfn.file"])
    ...
    def __init__(self, files=[]):
        super(BesDataset, self).__init__()
        self.files = files

    def __construct__(self, args):
        if (len(args) != 1) or (type(args[0]) is not type([])):
            super(BesDataset,self).__construct__(args)
        else:
            self.files = []
            for f in args[0]:
                if type(f) is type(''):
    ...
    def getReplicas(self):
        ...
        logger.error("cmd for replicas: %s", cmd)
        result = get_result(cmd,'LFC query error','Could not get replicas.')
        logger.error("replicas result: %s", result)
    ...
    def getCatalog(self,site=''):
        ...
        Note: site defaults to config.Boss.LocalSite
        ...
        if self.XMLCatalogueSlice.name:
            ...
            f.close()
            return xml_catalog
        if not site: site = getConfig('Boss')['LocalSite']
        lfns = self.getLFNs()
        depth = self.depth
    ...
    def optionsString(self,file=None):
        ...
        s = 'EventCnvSvc.digiRootInputFile = {'
      #  dtype_str_default = getConfig('Boss')['datatype_string_default']
      #  dtype_str_patterns = getConfig('Boss')['datatype_string_patterns']
        for f in self.files:
      ...
      #         if matched: break
            s += '\n'
            if isLFN(f):
                #put filename without directory since data is downloaded locally
                fname = os.path.split(f.name)
                s += """ \"%s\",""" % (fname[1])
            else:

...
class PythonOptionsParser:
    ...
    def _get_opts_dict_and_pkl_string( self):
        ...
        if stdout and rc == 0:
            try:
                ...
                #logger.error("zhang log: options:%s", options)
            except Exception, e:
    ...
    def _join_opts_files( self):
        ...
        for name in self.optsfiles:
            ...
            try:
                ...
                elif os.path.splitext( name)[1] == '.txt':
                    joined_py_opts += 'from Gaudi.Configuration import *\n'
                    joined_py_opts += 'importOptions(\'' + name + '\')\n'
                else:
                    msg = 'Only extensions of type ".opts"".py"".txt" allowed'
                    raise TypeError(msg)
            except IOError, e:
                logger.error('%s',e)
    ...
    def get_input_data( self):
        ...
        data = []
        try:
            data = [f for f in self.opts_dict['EventCnvSvc']['digiRootInputFile']]
        except KeyError, e:
    ...
    def get_output_files( self):        
        ...
        datatypes = ['EventCnvSvc','RootCnvSvc']
        for type in datatypes:
            if self.opts_dict.has_key(type):
               if self.opts_dict[type].has_key('digiRootOutputFile'):
                  f = self.opts_dict[type]['digiRootOutputFile']

...
option = sys.argv[1]
argv = sys.argv[2:]
commands = {'init':'dirac-proxy-init -g bes_user','info':'dirac-proxy-info',
...
def create_proxy_cache():
    ...
    if proxy_info is None: return False
    timeleft_str = None
...
def proxy_cache_is_valid():
    ...
    if not proxy_cache_file: return False
    if not os.path.exists(proxy_cache_file): return False
    cache_age = get_file_age(proxy_cache_file)
    proxy_age = get_file_age(get_proxy_file())
    if cache_age > 300 or cache_age > proxy_age: return False
    return True
